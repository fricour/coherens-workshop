...
# COHERENS workshop #2 {background-color="#DFA06E"}

Florian Ricour (ECOMOD) <br>
Ludovic Lepers (MFC)

::: footer
Information current as of November 13, 2024. Subject to change.
:::

# Part I {background-color="#DFA06E"}
...
# Short break {background-color="#DFA06E"}

# Part II {background-color="#DFA06E"}
...
<span style="background-color:#DFA06E; color:black; padding:10px 20px; border-radius:15px; display:inline-block; font-size:40px;">Artificial intelligence</span>
:::
:::
...
<span style="color: white;font-size: 24px;background-color:black;border-radius: 10px"><strong>Artificial Neural Network (ANN)</strong></span>
:::
:::
...
<span style="color: white;font-size: 24px;background-color:black;border-radius: 10px"><strong>$\rightarrow$ backpropagation algorithm (see after)</strong></span>
:::
:::
...
-   case 2: 4th dataset, show that we need a more complex network (take all neurons and all layers) to achieve a good result (keep only x1, x2 and sin(x1) and sin(x2) -> manifold learning, un peu comme un cinamon roll, with learning rate = 0.03, RELU and 80% de training data)
...
::: {.fragment .fade-in}
::: {style="position:absolute;top:40%;left:50%;"}
<span style="background-color:#DFA06E; color:black; padding:10px 20px; border-radius:15px; display:inline-block; font-size:40px;">Large Language Models (LLMs)</span>
:::
:::


# Part III {background-color="#DFA06E"}
...
::: {style="position:absolute;top:20%;left:0%;font-size:30px;"}
  - Large Langage Model &rarr; Huge model (neural network) with **billions** of parameters (GPT3 had 175 billion parameters)
...
## How do they works ? Transfomer principle (1)

- GPT => Generative pretrained transfomers
- Transformer : Neural network with many layer. 
- The model use as input a list of token called "context window"
- The model produce as output a token
- All the token pass trought the neural network and are modified based on the others token

- "A black dog" &rarr; dog vector will be modified to account for the fact that the dog is black (simplification) 

## How do they works ? Transfomer principle (2)

- At the end the model produce a vector transformed in a probability distribution. A token is chosen from there.
- When the previous token is added, the new context window can be used a new input => text can be generated "forever"
...
- Usually the model is better at retrieving information at the end of the context window
- This lead to two kinds of knowledge:
  - knowlegde learned during the training ("constant", stored in the parameters)
  - knowledge from the context window (different at each interacation)

## But how do we interact with a model?

1. There is a hidden system prompt that explain to the model that it must simulate a conversation
2. Then a sentence is given by the assistant
3. A sentence can then be given by the user, the "prompt"
4. Repeat 2 and 3

## How to make an LLM into a chatbot

- The model is "Fine tuned" &rarr; Reinforcement learning human feedback (RLHF)
- Human looks at many answer and says which one are good and which ones are bad (Necessary, cf Bing chat)
- This prevent the model to says "dangerous" stuff, but reduce its capacities a bit &rarr; *Models are trained to lure human to thinks that the answers looks good*
...
# Part IV {background-color="#DFA06E"}
...
# Time to eat ! {background-color="#DFA06E"}
